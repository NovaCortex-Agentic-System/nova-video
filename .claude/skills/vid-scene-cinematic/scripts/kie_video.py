#!/usr/bin/env python3
"""
kie_video.py — Generare video cinematic via KIE.ai (Kling) pentru NOVA Video agent.

Apelează direct API-ul KIE.ai fără dependință de MCP server.

Usage:
  python kie_video.py "<prompt>" <output_path> [opțiuni]
  python kie_video.py --health

Opțiuni:
  --model MODEL       Model KIE.ai (default: kling-3.0)
  --duration SECONDS  Durata video în secunde (default: 5)
  --aspect RATIO      Aspect ratio: 9:16 sau 16:9 (default: 9:16)
  --reference PATH    Imagine de referință pentru image-to-video
...
def generate(prompt, output_path, model, duration, aspect_ratio, reference_path=None):
    key = get_api_key()
...
def main():
    parser = argparse.ArgumentParser(description="Generare video cinematic Kling via kie.ai")
    ...
    parser.add_argument("--model", default="kling-3.0", help="Model KIE.ai (default: kling-3.0)")
    ...
    parser.add_argument("--reference", help="Cale imagine referință pentru image-to-video")
